...
// Con Ojos de Gato: 10% de probabilidad de hacer una tirada extra y quedarse con la mejor (más rara)
function getRamitaAleatoriaConSuerte() {
  const r1 = getRamitaAleatoria();
  if (Math.random() >= 0.10) return r1;
  const r2 = getRamitaAleatoria();
  return RAMITAS.indexOf(r1) >= RAMITAS.indexOf(r2) ? r1 : r2;
}
